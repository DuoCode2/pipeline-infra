...
```css
/* Good: Account for sticky headers when scrolling to focused elements */
:target {
  scroll-margin-top: 80px;
}

/* Good: Ensure focused items clear fixed/sticky bars */
:focus {
  scroll-margin-top: 80px;
...
/* Good: Comfortable target size (recommended 44x44) */
.touch-target {
  min-width: 44px;
  min-height: 44px;
  display: inline-flex;
