...
// Returns:
// {
//   comparisons: 27,  // 3 pages x 3 devices x 3 browsers
//   differences: 2,
...
def capture_themed_screenshots(page, url, name, selectors):
    ...
    for theme in ['dark', 'light']:
        ...
        if not content_loaded:
            print(f"  Warning: {theme} mode: Content slow to load, waiting...")
            time.sleep(2)
        ...
        status = 'OK' if results[f'{theme}_has_content'] else 'FAIL'
        print(f"  {theme}: {status} ({len(body_text)} chars)")
