import { business } from '@/data/business';
import { type Locale } from '@/lib/i18n';

function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`h-5 w-5 ${star <= rating ? 'text-yellow-400' : 'text-gray-300'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
    </div>
  );
}

export function Reviews({ locale }: { locale: Locale }) {
  const { reviews } = business.content[locale];
  const { theme } = business;
  const heading = {
    en: 'What Our Customers Say',
    ms: 'Apa Kata Pelanggan Kami',
    'zh-CN': '顾客评价',
    'zh-TW': '顧客評價',
  };

  return (
    <section className="py-16 sm:py-20" style={{ backgroundColor: theme.surface }}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mb-12 text-center">
          <h2 className="mb-2 font-display text-3xl font-bold sm:text-4xl" style={{ color: theme.textTitle }}>
            {heading[locale]}
          </h2>
          <div className="flex items-center justify-center gap-2">
            <StarRating rating={Math.round(reviews.rating)} />
            <span className="text-sm" style={{ color: theme.textBody }}>
              {reviews.rating.toFixed(1)} ({reviews.count}+)
            </span>
          </div>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {reviews.featured.map((review, i) => (
            <div
              key={i}
              className="rounded-xl bg-white p-6 shadow-sm"
            >
              <StarRating rating={review.rating} />
              <p className="mt-3 text-sm leading-relaxed" style={{ color: theme.textBody }}>
                &ldquo;{review.text}&rdquo;
              </p>
              <p className="mt-4 text-sm font-semibold" style={{ color: theme.textTitle }}>
                — {review.author}
              </p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
